...
static int GetEffectiveCustomAspect(bool ApplyCustomAspect)
{
	...
	if(AspectMode <= 0)
		return 0;
	return g_Config.m_MaCustomAspectRatio >= 100 ? g_Config.m_MaCustomAspectRatio : 0;
}
...
void CGraphics_Threaded::SetForcedAspect(bool Force, bool ApplyCustomAspect)
{
	...
	if(CustomAspect >= 100)
	{
		if(g_Config.m_MaCustomAspectRatioMode == 2 && g_Config.m_MaCustomAspectRatioNum > 0 && g_Config.m_MaCustomAspectRatioDen > 0)
			CustomAspectF = (float)g_Config.m_MaCustomAspectRatioNum / (float)g_Config.m_MaCustomAspectRatioDen;
		else
