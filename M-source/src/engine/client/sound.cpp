...
enum class EMaAudioFileFormat
{
	Wv,
	Wav,
};

static uint16_t ReadLittleEndian16(const unsigned char *pData)
{
	return (uint16_t)pData[0] | ((uint16_t)pData[1] << 8);
}

static uint32_t ReadLittleEndian32(const unsigned char *pData)
{
	return (uint32_t)pData[0] | ((uint32_t)pData[1] << 8) | ((uint32_t)pData[2] << 16) | ((uint32_t)pData[3] << 24);
}

static void ReplaceAudioExtension(const char *pFilename, const char *pExtension, char *pBuffer, int BufferSize)
{
...
bool CSound::DecodeWav(CSample &Sample, const void *pData, unsigned DataSize, const char *pContextName) const
{
	...
	if(DataSize < 12 || mem_comp(pBytes, "RIFF", 4) != 0 || mem_comp(pBytes + 8, "WAVE", 4) != 0)
	{
		log_error("sound/wav", "File is not a RIFF/WAVE file. Filename='%s'", pContextName);
		return false;
	}
	...
	int SampleRate = 0;
	int BitsPerSample = 0;
	const unsigned char *pAudioData = nullptr;
	unsigned AudioDataSize = 0;

	unsigned Position = 12;
	while(Position + 8 <= DataSize)
	{
		const unsigned char *pChunk = pBytes + Position;
		const unsigned ChunkSize = ReadLittleEndian32(pChunk + 4);
		Position += 8;
		if(ChunkSize > DataSize - Position)
		{
			log_error("sound/wav", "Invalid chunk size. Filename='%s'", pContextName);
			return false;
		}

		const unsigned char *pChunkData = pBytes + Position;
		if(mem_comp(pChunk, "fmt ", 4) == 0)
		{
			if(ChunkSize < 16)
			{
				log_error("sound/wav", "Invalid fmt chunk. Filename='%s'", pContextName);
				return false;
			}

			AudioFormat = ReadLittleEndian16(pChunkData);
			NumChannels = ReadLittleEndian16(pChunkData + 2);
			SampleRate = ReadLittleEndian32(pChunkData + 4);
			BitsPerSample = ReadLittleEndian16(pChunkData + 14);
		}
		else if(mem_comp(pChunk, "data", 4) == 0)
		{
			pAudioData = pChunkData;
	...
	if(AudioFormat != 1)
	{
		log_error("sound/wav", "Only PCM WAV is supported. Format=%d Filename='%s'", AudioFormat, pContextName);
		return false;
	}
	...
	if(BitsPerSample != 16)
	{
		log_error("sound/wav", "Bits per sample is %d, not 16. Filename='%s'", BitsPerSample, pContextName);
		return false;
	}
	if(SampleRate <= 0 || pAudioData == nullptr || AudioDataSize == 0)
	{
		log_error("sound/wav", "Missing audio data. Filename='%s'", pContextName);
		return false;
	}

	const int NumSamples = AudioDataSize / sizeof(short);
	const int NumFrames = NumSamples / NumChannels;
	if(NumFrames <= 0)
	{
		log_error("sound/wav", "No samples found. Filename='%s'", pContextName);
		return false;
	}

	Sample.m_pData = static_cast<short *>(calloc((size_t)NumFrames * NumChannels, sizeof(short)));
	for(int i = 0; i < NumFrames * NumChannels; ++i)
		Sample.m_pData[i] = static_cast<short>(ReadLittleEndian16(pAudioData + i * sizeof(short)));
...
int CSound::LoadOpus(const char *pFilename, int StorageType)
{
	...
	if(!Loaded && !m_pStorage->ReadFile(pFilename, StorageType, &pData, &DataSize))
	{
...
int CSound::LoadWV(const char *pFilename, int StorageType)
{
	...
	unsigned DataSize;
	bool Loaded = false;
	EMaAudioFileFormat LoadedFormat = EMaAudioFileFormat::Wv;
	char aLoadedPath[IO_MAX_PATH_LENGTH];
	aLoadedPath[0] = '\0';

	if(str_comp(g_Config.m_SndPack, "default") != 0)
	{
		auto TryLoad = [&](const char *pPath, EMaAudioFileFormat Format) {
			if(Loaded)
				return;
			if(m_pStorage->ReadFile(pPath, StorageType, &pData, &DataSize))
			{
				Loaded = true;
				LoadedFormat = Format;
				str_copy(aLoadedPath, pPath);
			}
		};

		auto TryCustomFilename = [&](const char *pCustomFilename) {
			char aCustomPath[IO_MAX_PATH_LENGTH];
			str_format(aCustomPath, sizeof(aCustomPath), "audio/%s/%s", g_Config.m_SndPack, pCustomFilename);
			TryLoad(aCustomPath, EMaAudioFileFormat::Wv);

			str_format(aCustomPath, sizeof(aCustomPath), "assets/audio/%s/%s", g_Config.m_SndPack, pCustomFilename);
			TryLoad(aCustomPath, EMaAudioFileFormat::Wv);

			char aWavFilename[IO_MAX_PATH_LENGTH];
			ReplaceAudioExtension(pCustomFilename, ".wav", aWavFilename, sizeof(aWavFilename));
			if(str_comp(aWavFilename, pCustomFilename) != 0)
			{
				str_format(aCustomPath, sizeof(aCustomPath), "audio/%s/%s", g_Config.m_SndPack, aWavFilename);
				TryLoad(aCustomPath, EMaAudioFileFormat::Wav);

				str_format(aCustomPath, sizeof(aCustomPath), "assets/audio/%s/%s", g_Config.m_SndPack, aWavFilename);
				TryLoad(aCustomPath, EMaAudioFileFormat::Wav);
			}
		};

		TryCustomFilename(pFilename);
		if(const char *pSuffix = str_startswith(pFilename, "audio/"))
			TryCustomFilename(pSuffix);
	}
	...
	const char *pContextName = aLoadedPath[0] ? aLoadedPath : pFilename;
	const bool DecodeSuccess = LoadedFormat == EMaAudioFileFormat::Wav ? DecodeWav(*pSample, pData, DataSize, pContextName) : DecodeWV(*pSample, pData, DataSize, pContextName);
	free(pData);
	if(!DecodeSuccess)
	{
		UnloadSample(pSample->m_Index);
