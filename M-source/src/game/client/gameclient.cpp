...
void CGameClient::InitializeLanguage()
{
	...
	// MΛ ツ
	str_format(aBuf, sizeof(aBuf), "ma/%s", g_Config.m_ClLanguagefile);
	g_Localization.Load(aBuf, Storage(), Console(), false);
}
...
void CGameClient::OnRender()
{
	...
	const bool UseGameNoHudAspect = IsActiveGameplay && g_Config.m_MaCustomAspectRatioApplyMode == 2;
	bool HudAspectDisabled = false;

	// render all systems
	for(auto &pComponent : m_vpAll)
	{
		if(g_Config.m_MaMusicVideoEffectBehind && pComponent == &m_Players)
			m_Ma.RenderMusicVideoEffectBackground();
...
{
	return {
		std::clamp(g_Config.m_MaBestInputOffset, 0, 1000),
		std::clamp(g_Config.m_MaBestInputSmoothing, 0, 100),
		std::clamp(g_Config.m_MaBestInputLatencyComp, 0, 50),
	};
}
...
float CGameClient::EffectiveFastInputOffsetTicks()
{
	...
	const int FastInputMode = MaFastInputNormalizedMode();
	if(FastInputMode == 0)
	{
	...
	if(FastInputMode == 4)
	{
		if(g_Config.m_MaSaikoPlusAmount <= 0)
			return 0.0f;
	...
	const SMaBestInputSettings Settings = MaBestInputSettings();
	if(Settings.m_Offset <= 0)
		return 0.0f;

	float Offset = Settings.m_Offset / 100.0f;
	if(Settings.m_Smoothing > 0)
		Offset *= 1.0f - (Settings.m_Smoothing / 200.0f);
	if(Settings.m_LatencyComp > 0)
		Offset *= 1.0f + (Settings.m_LatencyComp / 100.0f);
	return Offset;
}

static int FastInputPredictionTicks(float OffsetTicks)
{
	if(OffsetTicks <= 0.0f)
		return 0;
...
static bool EffectiveAnyFastInputOthers()
{
	const int FastInputMode = MaFastInputNormalizedMode();
	if(FastInputMode == 0)
...
static void ApplyFastInputOffset(float OffsetTicks, int &Tick, float &Intra)
{
	if(OffsetTicks <= 0.0f)
		return;
...
static float BestInputInterpolationAmount(float Fraction, float DeltaLength, bool Enable)
{
	if(!Enable)
		return Fraction;
...
static vec2 BestInputInterpolate(vec2 PrevPos, vec2 CurPos, float Fraction, bool Enable)
{
	return mix(PrevPos, CurPos, BestInputInterpolationAmount(Fraction, length(CurPos - PrevPos), Enable));
}

static float BestInputInterpolate(float PrevPos, float CurPos, float Fraction, bool Enable)
{
...
void CGameClient::OnPredict()
{
	...
	for(int Tick = Client()->GameTick(g_Config.m_ClDummy) + 1; Tick <= FinalTickSelf; Tick++)
	{
		...
		if(FastInputTicks > 0 && Tick > FinalTickRegular)
		{
			...
			if(MaFastInputNormalizedMode() != 4 && GetDummyFastInput(DummyFastInput, pDummyInputData, pDummyChar, LocalTee, DummyTee))
...
vec2 CGameClient::GetSmoothPos(int ClientId)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if(in_range(TimePassed, (int64_t)0, Len - 1))
		{
			...
			if(!IsLocalFastInputClient && FastInputTicksClient > 0 && FastInputOthers)
...
void CGameClient::LoadCursorAsset(const char *pPath, bool AsDir)
{
	...
	const auto LoadDefaultCursor = [this]() {
		...
		if(m_CursorTextureOverride.IsNullTexture() && g_pData->m_aImages[IMAGE_CURSOR].m_pFilename[0] != '\0')
...
void CGameClient::LoadArrowAsset(const char *pPath, bool AsDir)
{
	...
	const auto LoadDefaultArrow = [this]() {
		...
		if(m_ArrowTextureOverride.IsNullTexture() && g_pData->m_aImages[IMAGE_ARROW].m_pFilename[0] != '\0')
...
{
	if(m_Ma.IsComponentDisabled(0))
		return false;
	return g_Config.m_MaOptimizer != 0;
}

bool CGameClient::OptimizerDisableParticles() const
{
	return OptimizerEnabled() && g_Config.m_MaOptimizerNoParticles != 0;
}

bool CGameClient::OptimizerFpsFogEnabled() const
{
	return OptimizerEnabled() && g_Config.m_MaOptimizerFpsFog != 0;
}
...
void CGameClient::OptimizerFpsFogHalfExtents(float &HalfW, float &HalfH) const
{
	...
	{
		const float Radius = (float)g_Config.m_MaOptimizerFpsFogRadiusTiles * 32.0f;
		HalfW = Radius;
		HalfH = Radius;
		return;
	}
	...
	float Height = 0.0f;
	Graphics()->CalcScreenParams(Graphics()->ScreenAspect(), m_Camera.m_Zoom, &Width, &Height);
	const float Percent = std::clamp(g_Config.m_MaOptimizerFpsFogZoomPercent, 10, 120) / 100.0f;
...
#if defined(CONF_FAMILY_WINDOWS)
static bool SetOptimizerPriorityClassForPid(DWORD Pid, DWORD PriorityClass)
{
	const HANDLE Process = OpenProcess(PROCESS_SET_INFORMATION | PROCESS_QUERY_LIMITED_INFORMATION, FALSE, Pid);
	if(Process == nullptr)
		return false;
	const bool Success = SetPriorityClass(Process, PriorityClass) != 0;
	CloseHandle(Process);
	return Success;
}

static int SetOptimizerPriorityClassForProcessNames(const wchar_t *const *apExeNames, size_t NumNames, DWORD PriorityClass)
{
	...
	if(Process32FirstW(Snapshot, &Entry))
	{
		do
		{
			...
			{
				if(wcscmp(Entry.szExeFile, apExeNames[i]) == 0)
				{
					if(SetOptimizerPriorityClassForPid(Entry.th32ProcessID, PriorityClass))
						Count++;
					break;
				}
			}
...
{
#if defined(CONF_FAMILY_WINDOWS)
	const bool WantDdnetHigh = OptimizerEnabled() && g_Config.m_MaOptimizerHighPriority != 0;
	...
	const bool WantDiscordBelow = OptimizerEnabled() && g_Config.m_MaOptimizerDiscordPriorityBelowNormal != 0;
	if(WantDiscordBelow && !m_OptimizerDiscordPriorityBelowNormalActive)
	{
		SetOptimizerPriorityClassForProcessNames(s_apDiscordExeNames, std::size(s_apDiscordExeNames), BELOW_NORMAL_PRIORITY_CLASS);
	...
	{
		SetOptimizerPriorityClassForProcessNames(s_apDiscordExeNames, std::size(s_apDiscordExeNames), NORMAL_PRIORITY_CLASS);
...
void CGameClient::RenderOptimizerFpsFogRect()
{
	...
	if(!OptimizerFpsFogEnabled() || g_Config.m_MaOptimizerFpsFogRenderRect == 0)
		return;
	...
	const CMapItemGroup *pGameGroup = Layers()->GameGroup();
	if(pGameGroup)
	{
