...
static void LoadCursorPreview(CMenus::SCustomCursor *pCursorItem, IGraphics *pGraphics)
{
	...
	{
		pCursorItem->m_RenderTexture = pGraphics->LoadTexture("data/gui_cursor.png", IStorage::TYPE_ALL);
		if(pCursorItem->m_RenderTexture.IsNullTexture())
			pCursorItem->m_RenderTexture = pGraphics->LoadTexture("gui_cursor.png", IStorage::TYPE_ALL);
		if(pCursorItem->m_RenderTexture.IsNullTexture() && g_pData->m_aImages[IMAGE_CURSOR].m_pFilename[0] != '\0')
			pCursorItem->m_RenderTexture = pGraphics->LoadTexture(g_pData->m_aImages[IMAGE_CURSOR].m_pFilename, IStorage::TYPE_ALL);
		if(pCursorItem->m_RenderTexture.IsNullTexture())
...
int CMenus::CursorScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	if(IsDir)
	{
		if(pName[0] == '.' || str_comp(pName, "default") == 0 || Exists(pName))
			return 0;
	...
	else if(str_endswith(pName, ".png"))
	{
		char aName[IO_MAX_PATH_LENGTH];
		str_truncate(aName, sizeof(aName), pName, str_length(pName) - 4);
		if(str_comp(aName, "default") == 0 || Exists(aName))
			return 0;
...
static void LoadArrowPreview(CMenus::SCustomArrow *pArrowItem, IGraphics *pGraphics)
{
	...
	{
		pArrowItem->m_RenderTexture = pGraphics->LoadTexture("data/arrow.png", IStorage::TYPE_ALL);
		if(pArrowItem->m_RenderTexture.IsNullTexture())
			pArrowItem->m_RenderTexture = pGraphics->LoadTexture("arrow.png", IStorage::TYPE_ALL);
		if(pArrowItem->m_RenderTexture.IsNullTexture() && g_pData->m_aImages[IMAGE_ARROW].m_pFilename[0] != '\0')
			pArrowItem->m_RenderTexture = pGraphics->LoadTexture(g_pData->m_aImages[IMAGE_ARROW].m_pFilename, IStorage::TYPE_ALL);
		if(pArrowItem->m_RenderTexture.IsNullTexture())
...
void CMenus::RenderSettingsCustom(CUIRect MainView)
{
	...
	{
		if(m_vArrowList.empty())
		{
			SCustomArrow ArrowItem;
			str_copy(ArrowItem.m_aName, "default");
			LoadArrowPreview(&ArrowItem, Graphics());
			m_vArrowList.push_back(ArrowItem);
			Storage()->ListDirectory(IStorage::TYPE_ALL, "assets/arrow", ArrowScan, &User);
			std::sort(m_vArrowList.begin(), m_vArrowList.end());
		}
