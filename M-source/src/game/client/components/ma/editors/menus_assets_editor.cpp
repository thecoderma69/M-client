...
static bool AssetsEditorLoadPreview(IGraphics *pGraphics, CMenus::SAssetsEditorAssetEntry &Entry)
{
	CImageInfo PreviewInfo;
	if(!pGraphics->LoadPng(PreviewInfo, Entry.m_aPath, IStorage::TYPE_ALL))
		return false;
	...
	if(Entry.m_PreviewWidth <= 0 || Entry.m_PreviewHeight <= 0)
		return false;

	Entry.m_PreviewTexture = pGraphics->LoadTexture(Entry.m_aPath, IStorage::TYPE_ALL);
	if(!Entry.m_PreviewTexture.IsValid())
	{
		pGraphics->UnloadTexture(&Entry.m_PreviewTexture);
		return false;
	}
...
static int AssetsEditorScanCallback(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	if(!AssetsEditorLoadPreview(pContext->m_pGraphics, Entry))
		return 0;
...
void CMenus::AssetsEditorReloadAssets()
{
	...
	auto ReloadType = [this](std::vector<SAssetsEditorAssetEntry> &vAssets, int Type) {
		...
		if(AssetsEditorLoadPreview(Graphics(), DefaultAsset))
