...
void CCamera::OnRender()
{
	...
	if(g_Config.m_TcVisualsZoom)
	{
		const int CustomZoomLevel = std::clamp(g_Config.m_TcVisualsZoomLevel, 1, 20);
		m_Zoom = std::clamp(CCamera::ZoomStepsToValue(CustomZoomLevel - 10.0f), MinZoomLevel(), MaxZoomLevel());
		m_UserZoomTarget = m_Zoom;
		m_Zooming = false;
		m_AutoSpecCameraZooming = false;
	}
