...
void CMenus::OnInit()
{
	...
	if(g_Config.m_MaMenuMediaBackgroundPath[0] == '\0')
	{
		str_copy(g_Config.m_MaMenuMediaBackgroundPath, "tclient/backgrounds/thumb-1920-735980.png", sizeof(g_Config.m_MaMenuMediaBackgroundPath));
		g_Config.m_MaMenuMediaBackground = 1;
	}
...
{
	char aOldPack[sizeof(g_Config.m_SndPack)];
	str_copy(aOldPack, g_Config.m_SndPack);
	pfnCallback(pResult, pCallbackUserData);

	auto *pSelf = static_cast<CMenus *>(pUserData);
	if(pResult->NumArguments() && str_comp(aOldPack, g_Config.m_SndPack) != 0)
		pSelf->GameClient()->m_Sounds.Clear();
}
...
void CMenus::Render()
{
	...
	else
	{
		...
		m_MenuMediaBackground.SyncFromConfig(g_Config.m_MaMenuMediaBackground, g_Config.m_MaMenuMediaBackgroundPath);
...
{
	TextRender()->DeleteTextContainer(m_MotdTextContainerIndex);
	m_MenuMediaBackground.OnWindowResize();
}
...
void CMenus::OnRender()
{
	...
	const bool UseWindowAspectForUi = IngameMenu && g_Config.m_MaCustomAspectRatioApplyMode != 1;
