	...
	float Intra)
{
	...
	if(g_Config.m_TcVisualsTransparentTees && !Local && ClientId >= 0 && !Spec)
		Alpha = (100 - g_Config.m_TcVisualsTransparentAmount) / 100.0f;
	...
	// draw gun
	if(Player.m_Weapon >= 0 && !g_Config.m_TcVisualsNoRenderWeapons)
	{
...
void CPlayers::OnRender()
{
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		...
		if(!g_Config.m_TcVisualsNoHookLine)
