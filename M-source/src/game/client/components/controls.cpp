...
int CControls::ResolveSnapTapDirection(int Dummy, bool LeftPressed, bool RightPressed)
{
	if(LeftPressed == RightPressed)
	{
		...
		{
			const int64_t Delay = (time_freq() * (int64_t)g_Config.m_MaSnapTapDelay) / 1000;
...
void CControls::OnRender()
{
	...
	if(g_Config.m_TcMiscAutoJump && m_aInputData[g_Config.m_ClDummy].m_Jump)
	{
...
void CControls::GoresMode()
{
	if(!GameClient()->m_Snap.m_pLocalCharacter)
		return;
	if(!g_Config.m_MaEnabled || !g_Config.m_MaGoresMode)
		return;
	if(GameClient()->m_Ma.IsComponentDisabled(1))
		return;

	CCharacterCore Core = GameClient()->m_PredictedPrevChar;

	if(g_Config.m_MaGoresModeDisableIfWeapons)
	{
