...
class CChat : public CComponent
{
	...
	CLine m_aHistoryLines[MAX_HISTORY];
	int m_HistoryCurrent;
	int m_HistoryCount;
	bool m_HistoryMode;
	int m_HistoryScrollOffset;
	bool m_SearchActive;
	char m_aSearchText[64];
	std::vector<int> m_vSearchMatches;
	int m_CurrentSearchMatch;

	void RenderHistoryPanel();
	void UpdateSearch();
	void AddHistoryLine(const CLine &Line);
	static void ConShowChatHistory(IConsole::IResult *pResult, void *pUserData);
