...
{
	Graphics()->WrapClamp();
	IGraphics::CTextureHandle CursorTexture = g_pData->m_aImages[IMAGE_CURSOR].m_Id;
	if(m_pGameClient)
		CursorTexture = m_pGameClient->CursorTexture();
	Graphics()->TextureSet(CursorTexture);
	Graphics()->QuadsBegin();
	Graphics()->QuadsSetSubset(0, 0, 1, 1);
	...
	for(int Pass = 0; Pass < 2; Pass++)
	{
		...
		for(int Filling = 0; Filling < 2; Filling++)
		{
			...
			if(Filling == 1)
			{
				...
				if(pInfo->m_UseCustomEyeColors)
				{
					Graphics()->SetColor(pInfo->m_ColorEyeLeft.r, pInfo->m_ColorEyeLeft.g, pInfo->m_ColorEyeLeft.b, Alpha);
					...
					Graphics()->SetColor(pInfo->m_ColorEyeRight.r, pInfo->m_ColorEyeRight.g, pInfo->m_ColorEyeRight.b, Alpha);
					Graphics()->RenderQuadContainerAsSprite(m_TeeQuadContainerIndex, QuadOffset + EyeQuadOffset, BodyPos.x + EyeSeparation + Offset.x, BodyPos.y + Offset.y, -EyeScale / (64.f * 0.4f), h / (64.f * 0.4f));
				}
				else
				{
